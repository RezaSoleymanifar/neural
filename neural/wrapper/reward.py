...
class RewardGeneratorWrapper(RewardWrapper):
    """
    A wrapper that generates rewards for the environment. By default the market
    env returns None as reward. This wrapper combined with the metadata wrapper
    provide the reward signal which is the change in equity from the previous
    step. Equity is defined as the net value owned by the agent in cash and
    assets. This is sum of all cash and assets owned by the agent minus cash
    and asset debt. E = L + C - S where E is equity, L total value of longs, C
    cash, S total value of shorts. Note that cash can be negative if the agent
    has borrowed cash.
...
class RewardNormalizerWrapper(RewardWrapper):
    """
    This wrapper will normalize immediate rewards. This should typically be the
    last wrapper in the reward wrapper stack. This wrapper normalizes immediate
    rewards so that rewards have mean 0 and standard deviation 1.
    ...
    def __init__(
        ...
        """
        This wrapper normalizes immediate rewards so that rewards have mean 0
        ...
        -------
            ...
            clip_threshold (float):
                The maximum value to clip the normalized reward. This is useful
                to prevent the agent from receiving very large rewards.
    ...
    def reward(self, reward: float) -> float:
        """
        Normalize the reward. This method should be the last wrapper in the
        reward wrapper stack.
...
class LiabilityInterstRewardWrapper(RewardWrapper):
    ...
    borrowed assets. The interest rate is calculated as a percentage of the
    liability. Apply this wrapper prior to normalization of rewards as this
    substracts the notional value of interest from the reward. Applies the
    ...
    -------
        env (gym.Env):
            The environment to wrap.
        interest_rate (float):
            The interest rate to charge on liabilities. This is expressed as a
            percentage of the liability. The interest rate is applied daily.
            The default value is 8% per annum.
    ...
    -----------
        ...
        previous_day (datetime.date):
            The day of the previous step. This is used to determine if the day
            has changed and interest should be charged.
    ...
    def reward(self, reward: float) -> float:
        ...
        --------
            float:
                The modified reward. Subtracts the interest from the reward.
        """
    ...
    def compute_interest(self) -> float:
        """
        Compute the interest to charge on liabilities. Liabilities include
        borrowed cash, or borrowed assets. The interest rate is applied on a
        daily basis.
...
class AbstractRewardShaperWrapper(RewardWrapper, ABC):
    ...
    This class is designed to be subclassed for creating custom reward shaping
    wrappers for market environments. Reward shaping wrappers are used to
    modify the reward signal obtained by an agent in order to encourage or
    discourage certain behaviours during training. highly useful for
    pretraining an agent with some degrees of freedom in actions. Apply
    relevant reward shaping wrappers to define and restrict unwanted actions.
    Start with a pipe of wrappers that enforce the desired behaviour and later
    remove the influencing wrappers to allow the agent to learn the desired
    behaviour. if desired behavior is a starting point, then in a final step
    remove the reward shaping wrapper and the agent may learn to improve on it.
    ...
    def check_condition(self, *args, **kwargs) -> bool:
        ...
        This method should be implemented by subclasses to determine whether to
        apply reward shaping to the current step. The method takes an arbitrary
        number of arguments and keyword arguments, depending on the specific
    ...
    def reward(self, reward: float, *args, **kwargs) -> float:
        ...
        This method should be implemented by subclasses to modify the reward
        signal based on the current episode state. The method takes the current
        reward as input, and an arbitrary number of arguments and keyword
        arguments, depending on the specific reward shaping strategy.
    ...
    def parse_scale(self,
        ...
        -----
            ...
            value (float):
                ...
                threshold = excess_margin_ratio_threshold > 0 then the scaling
                factor will be greater than 1. This way agent learns to avoid
                margin calls.
            factor (float):
                factor serves two purposes:
                    - sign of factor determines punishment/encouragement.
...
class AbstractFixedRewardShaperWrapper(AbstractRewardShaperWrapper):
    ...
    This class defines the interface for a fixed reward shaper wrapper, which
    shapes the reward signal of an environment based on a threshold value. To
    create a custom fixed reward shaper, users must inherit from this class and
    ...
    Methods:
        ...
        step(action) -> tuple:
            Takes a step in the environment and returns the observation, shaped
            reward, done flag, and info dictionary.
    """
    ...
    def __init__(
        ...
        -----
            ...
            optional): 
                Whether to use the standard deviation of the rewards. Defaults
                to None.
            use_min (bool or None, optional): 
                Whether to use the min/max reward statistics. Defaults to None.
                if use_min = Flase, then with default scale = 1 the shaped
                reward will be -1 * max reward meaning if reward condition is
                met the shaped reward will be the negative maximum reward. 
...
class AbstractDynamicRewardShaperWrapper(AbstractRewardShaperWrapper, ABC):
    ...
    This class defines the interface for a dynamic reward shaper wrapper, which
    shapes the reward signal of an environment based on a dynamically adjusted
    threshold value. To create a custom dynamic reward shaper, users must
    inherit from this class and implement the abstract methods:
    `check_condition`, `metric`, and `threshold`.
    ...
    Attributes:
        ...
        shaped reward. Defaults to 1.0. factor (float, optional): The factor
        used to adjust the scaling factor. Defaults to 1.0. base (float,
        optional): The base value used in the scaling factor adjustment.
        Defaults to 1.0.
    ...
    def __init__(
        ...
        Args:
            ...
            optional): The factor used to adjust the scaling factor. Defaults
            to -1.0. when factor > 0 the shaped reward will be positive. When
            factor < 0 the shaped reward will be negative. base (float,
            optional): The base value used in the scaling factor adjustment.
            Defaults to 1.0.
        """
    ...
    def check_condition(self) -> bool:
        """
        Abstract method that checks whether the reward should be shaped based
        on the current episode state.
    ...
    def metric(self) -> float:
        """
        Abstract property that defines the metric used to adjust the scaling
        factor.
    ...
    def reward(self, reward: float) -> float:
        """
        Shapes the reward signal based on the check_condition method and the
...
class FixedPenalizeShortRatioRewardWrapper(AbstractFixedRewardShaperWrapper):
    """
    A reward shaping wrapper that penalizes a short ratio lower than a given
    threshold.

    Args:
        env (gym.Env): The environment to wrap. short_ratio_threshold (float):
        The maximum short ratio allowed before being penalized.

    Attributes:
        env (gym.Env): The environment being wrapped. reward_rms
        (RunningMeanStandardDeviation): The running mean and standard deviation
        object for tracking reward statistics. market_metadata_wrapper
        (MarketMetadataWrapper): The metadata wrapper for the market
        environment. short_ratio_threshold (float): The maximum short ratio
...
class DynamicPenalizeShortRatioRewardWrapper(
    ...
    This class modifies the reward signal of a market environment by applying a
    penalty when the short ratio exceeds a specified threshold. The penalty is
    based on the deviation of the short ratio from the threshold.
    ...
    factor : float, optional
        A factor used to modify the penalty based on the deviation of the short
        ratio from the threshold. Default is 1.0.
    use_std : bool, optional
        A boolean indicating whether to use the reward's standard deviation in
        shaping the reward. Default is None.
    use_min : bool, optional
        A boolean indicating whether to use the maximum reward value in shaping
        the reward. Default is None. Alternative is to use the maximum reward
        value.
    scale : float, optional
        A float value used to scale the shaped reward based on chosen method.
        Default is 1.
    ...
    reward_rms : RunningMeanStandardDeviation
        The running mean and standard deviation object for tracking reward
        statistics.
    ...
    check_condition() -> bool:
        Checks whether the reward should be shaped based on the current episode
        state.
    ...
    def __init__(
        ...
        env : gym.Env
            The environment to wrap.
        short_ratio_threshold : float, optional
            The maximum short ratio allowed before being penalized. Default is
            0.2.
        factor : float, optional
            A factor used to modify the penalty based on the deviation of the
            short ratio from the threshold. Default is -1.0.
        use_std : bool, optional
...
class FixedPenalizeCashRatioRewardWrapper(AbstractRewardShaperWrapper):
    """
    A reward shaping wrapper that penalizes a cash ratio lower than a given
    threshold. The cash ratio is defined as the ratio of cash to net worth.
    This ratio has meaning when both cash and net worth are positive.

    This class modifies the reward signal of a market environment by applying a
    penalty when the cash ratio rises above a specified threshold. The penalty
    is based on the deviation of the cash ratio from the threshold.
    ...
    factor : float, optional
        A factor used to modify the penalty based on the deviation of the cash
        ratio from the threshold. Default is -1.0.
    use_std : bool, optional
        A boolean indicating whether to use the reward's standard deviation in
        shaping the reward. Default is None.
    ...
    def __init__(
        ...
        env : gym.Env
            The environment to wrap.
        cash_ratio_threshold : float, optional
            The maximum cash ratio allowed before being penalized. Default is
            0.01.
        factor : float, optional
            A factor used to modify the penalty based on the deviation of the
            cash ratio from the threshold. Default is -1.0.
        use_std : bool, optional
            A boolean indicating whether to use the reward's standard deviation
            in shaping the reward. Default is None.
        use_min : bool, optional
...
class DynamicPenalizeShortRatioRewardWrapper(
    ...
    def __init__(
        ...
        Args:
            env (Env): The environment to wrap. cash_ratio_threshold (float,
            optional): The maximum threshold for the ratio of cash to net
            worth. If the ratio rises above this threshold, the agent will be
            penalized. Defaults to 0.1. use_std (bool, optional): Indicates
            whether to use standard deviation or not. Defaults to None. use_min
            (bool, optional): Indicates whether to use the maximum or not.
            Defaults to None. factor (float, optional): The factor by which the
            reward will be penalized. Defaults to -1.0.

        Attributes:
            cash_ratio_threshold (float): The maximum threshold for the ratio
            of cash to net worth. cash_ratio (float): The current cash ratio.
        ...
        Methods:
            check_condition() -> bool: Calculates the current cash ratio by
            dividing the amount of cash by the net worth. If the cash ratio
            rises above the threshold, the method returns True, indicating that
            the agent should be penalized.
        """
        ...
        Args:
            env (Env): The environment to wrap. cash_ratio_threshold (float,
            optional): The maximum threshold for the 
                ratio of cash to net worth. If the ratio rises above this
                threshold, the agent will be penalized. Defaults to 0.1.
            use_std (bool, optional): Indicates whether to use standard
            deviation or not. Defaults to None. use_min (bool, optional):
            Indicates whether to use the maximum or not. Defaults to None.
            factor (float, optional): The factor by which the reward will be
    ...
    def check_condition(self) -> bool:
        """
        Calculates the current cash ratio by dividing the amount of cash by the
        net worth. If the net worth or cash is zero or negative, returns False.
        If the cash ratio rises above the threshold, the method returns True,
        indicating that the agent should be penalized.

        Returns:
            bool: True if the cash ratio rises above the threshold, False
            otherwise.
        """
